...
def test_ft_exercise(exercise_file_name):
    ...
    - Look for the correct folder (ex1/ft_plot_area.py)
    - Import it
    - Call the function ft_plot_area() inside it
    """
    print(f"\n=== Testing {exercise_file_name} ===")

    # Map exercises to their folders
    exercise_map = {
        "ft_hello_garden": "ex0",
        "ft_plot_area": "ex1",
    ...
    try:
        folder = exercise_map.get(exercise_file_name)
        if not folder:
            raise ImportError(f"Unknown folder for {exercise_file_name}")

        # Import your exercise file dynamically
        module_name = f"{folder}.{exercise_file_name}"
