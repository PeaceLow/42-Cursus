...
class Small_LLM_Model:
    ...
    def get_logits_from_input_ids(self, input_ids: list[int]) -> list[float]:
        ...
        """
        # If input_ids just grew by 1 token and we have past_key_values, we only need to pass the new token!
        if self._past_key_values is not None and self._last_input_ids is not None and len(input_ids) == len(self._last_input_ids) + 1 and input_ids[:-1] == self._last_input_ids:
            input_tensor = torch.tensor([[input_ids[-1]]], device=self._device, dtype=torch.long)
            with torch.no_grad():
                out = self._model(input_ids=input_tensor, past_key_values=self._past_key_values, use_cache=True)
        else:
            # We must recompute or this is the first token
        ...
        self._last_input_ids = input_ids.copy()
